...
import {
  ...
  CheckCircle2, XCircle, GraduationCap, Calendar,
  Library, ClipboardList,
} from 'lucide-react';
import { useAuthStore } from '@/store/auth';
import { dashboardApi, type WindowDays } from '@/lib/dashboard/api';
import {
  StatCard, DashBarChart, DashDonut, DashAreaChart, DashPanel,
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-200">
        <div>
          <div className="flex items-center gap-1.5 text-orange-500 text-[11px] font-bold uppercase tracking-widest mb-1">
            <GraduationCap className="w-3.5 h-3.5" /> Trainer Dashboard
          </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="Total Courses"    value={h.total_courses}     icon={<GraduationCap className="w-4 h-4" />} accent="amber" />
        <StatCard label="Student Count"    value={h.student_count}     icon={<Users className="w-4 h-4" />}        accent="green" />
        <StatCard
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          <DashPanel
            ...
            subtitle="Top 8 courses — enrolled + avg progress"
            accent="brand"
            right={
              <Link href="/trainer/courses" className="text-xs text-orange-600 hover:text-orange-700 font-semibold">Manage →</Link>
            }
          >
            {perCourseData.length === 0
              ? <EmptyChart msg="No enrolled students yet. Publish a course to start." />
              : <DashBarChart data={perCourseData} xKey="label" height={260} bars={[
                  { dataKey: 'students',       name: 'Students',       color: COLORS.brand },
                  { dataKey: 'Avg Progress %', name: 'Avg Progress %', color: COLORS.amber },
                ]} />}
          </DashPanel>
        </div>
      ...
      <DashPanel
        ...
        accent="brand"
        right={
          <Link href="/trainer/quizzes" className="text-xs text-orange-600 hover:text-orange-700 font-semibold">All quizzes →</Link>
        }
      >
              ...
              {data.recent_activity.map((a, i) => (
                <li key={i} className="py-3 flex items-center gap-3">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${
                    a.type === 'enrollment' ? 'bg-orange-50'
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <QuickLink href="/trainer/courses"        icon={<BookOpen className="w-5 h-5" />}     title="Manage Courses"   subtitle="Modules · Lessons · Assignments" color="bg-orange-50 text-orange-500" />
          <QuickLink href="/trainer/quizzes"        icon={<Zap className="w-5 h-5" />}          title="Manage Quizzes"   subtitle="Live · Self-paced · Exams"       color="bg-amber-50 text-amber-600" />
